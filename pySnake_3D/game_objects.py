...
class Snake:
    def __init__(self, MAP_SIZE):
        ...
        self.segment_positions = [Vec3(randrange(MAP_SIZE) + 0.5,
                                        randrange(MAP_SIZE) + 0.5,
                                        -0.5)]
        self.directions = {'a': Vec3(-1, 0, 0),
                            'd': Vec3(1, 0, 0),
                            'w': Vec3(0, 1, 0),
                            's': Vec3(0, -1, 0),}
        self.direction = Vec3(0, 0, 0)
        self.permissons = {'a': 1, 'd': 1, 'w': 1, 's': 1}
    ...
    def control(self):
        for key in 'wasd':
            if held_keys[key] and self.permissons[key]:
                self.direction = self.directions[key]
                self.permissions = dict.fromkeys(self.permissons, 1)
